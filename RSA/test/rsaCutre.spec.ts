...
describe('test del objeto rsaCutre',()=>{
    ...
    user1.keyNumber = new bigNum('314082567765085892950146723820903205');
    user1.mod = new bigNum('644681300498619888408807985830174569')
    user1.phi = new bigNum('644681300498619880556038893765872196');
    user1.publicKey = new PublicKey();
    user1.publicKey.mod = user1.mod;
    user1.publicKey.keyNumber = new bigNum(65537);

    let user2 = new PrivateKey();
    user2.keyNumber = new bigNum('424066677481442193977145250950415073');
    user2.mod = new bigNum('820187629986757477587598236238963847')
    user2.phi = new bigNum('820187629986757475776307165752909920');
    user2.publicKey = new PublicKey();
    user2.publicKey.mod = user2.mod;
    user2.publicKey.keyNumber = new bigNum(65537);

    it('prueba la funcion expConMod',()=>{
        expect(expConMod(new bigNum(3),new bigNum(4),new bigNum(16)).toString()).to.equal('1');
    })
    ...
    it('prueba de la funcion messageToBigNum',async()=>{
        expect(messageToBigNum('HOLA paTiTO!')).to.be.deep.equal(new bigNum('22378785948114938758920687393'));
    })
    it('prueba de la funcion bigNumToMessage',async()=>{
        expect(bigNumToMessage(new bigNum('22378785948114938758920687393'))).to.be.deep.equal('HOLA paTiTO!');
    })
    ...
    it('prueba de la funcion decrypt',async()=>{
        let decrypted = await user1.decrypt(new bigNum('505440789974123141460027844142835956'));
        expect(bigNumToMessage(decrypted)).to.be.equal('hello node!');
    })
    it('prueba la funcion sign',async()=>{
        let signed = await user2.sign(new bigNum('505440789974123141460027844142835956'));
        expect(signed.toString()).to.be.equal('644713203552166444474272534666011733');

    })
    it('prueba la funcion verify',async()=>{
        let verified = await user2.publicKey.verify(new bigNum('644713203552166444474272534666011733'));
